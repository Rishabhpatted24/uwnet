...
matrix variance(matrix x, matrix m, int groups)
{
    ...
    for(i = 0; i < v.cols; ++i){
        v.data[i] = 1.0 * v.data[i] / x.rows / n;
    }
    return v;
}
...
matrix delta_mean(matrix d, matrix v)
{
    ...
    for(i = 0; i < dm.cols; ++i){
        dm.data[i] = 1.0 * dm.data[i] / dm.rows;
    }
    return dm;
}
...
matrix delta_variance(matrix d, matrix x, matrix m, matrix v)
{
    ...
    for(i = 0; i < d.rows; ++i){
        for(j = 0; j < d.cols; ++j){
            dv.data[j/n] += (d.data[i*d.cols + j] * (x.data[i*x.cols + j] - m.data[j/n]) * -0.5 * pow((v.data[j/n] + eps), -1.5));
        }
    }
    for(i = 0; i < dv.cols; ++i){
        dv.data[i] = 1.0 * dv.data[i] / dv.rows;
    }
    return dv;
}
